  ...
  return (
    <div className={`${theme === 'dark' ? 'bg-gray-900' : 'bg-[#F6F8FF]'} min-h-screen py-10 transition-all`}>
      <div className="max-w-2xl mx-auto px-4 flex flex-col gap-8">
        ...
        {/* Error Message */}
        {error && (
          <p className="text-center text-yellow-500 font-semibold">{error}</p>
        )}
                ...
                <div className="flex-1">
                  <div className="flex flex-col md:flex-row md:justify-between">
                    ...
                    <time className="text-gray-400 text-sm">
                      {user.created_at && `Joined ${new Date(user.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}`}
                    </time>
                  </div>
